...
function handleCityGoldPricesPage() {
    ...
    if (actualCityName) {
        document.getElementById('cityTitle').textContent = `Gold Prices in Punjab`;
...
async function fetchAndDisplayCityGoldPrices(city) {
    ...
    try {
        ...
        if (todayCityData && yesterdayCityData) {
            const goldPricesDiv = document.getElementById('goldPricesPunjab');
            goldPricesDiv.innerHTML = `
                <div class="mb-8">
                    <p class="mb-4">Get real-time gold prices for Punjab 24K, 22K, and 18K Gold per gram in INR.</p>
