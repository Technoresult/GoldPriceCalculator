...
function handleCityGoldPricesPage() {
    ...
    if (actualCityName) {
        document.getElementById('cityTitle').textContent = `Gold Prices in Karnataka`;
...
async function fetchAndDisplayCityGoldPrices(city) {
    ...
    try {
        ...
        if (todayCityData && yesterdayCityData) {
            const goldPricesDiv = document.getElementById('goldPricesKarnataka');
            goldPricesDiv.innerHTML = `
                <div class="mb-8">
                    <p class="mb-4">Get real-time gold prices for Karnataka 24K, 22K, and 18K Gold per gram in INR.</p>
