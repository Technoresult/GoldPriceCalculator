...
function handleCityGoldPricesPage() {
    ...
    if (actualCityName) {
        document.getElementById('cityTitle').textContent = `Gold Prices in Maharashtra`;
...
async function fetchAndDisplayCityGoldPrices(city) {
    ...
    try {
        ...
        if (todayCityData && yesterdayCityData) {
            const goldPricesDiv = document.getElementById('goldPricesMaha');
            goldPricesDiv.innerHTML = `
                <div class="mb-8">
                    <p class="mb-4">Get real-time gold prices for Maharashtra 24K, 22K, and 18K Gold per gram in INR.</p>
