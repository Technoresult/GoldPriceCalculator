...
function handleCityGoldPricesPage() {
    ...
    if (actualCityName) {
        document.getElementById('cityTitle').textContent = `Gold Prices in Andhra`;
...
function populateGoldSidebar() {
    ...
    const topCities = ['Amaravati', 'Anantapur', 'Guntur', 'Kadapa', 'Kakinada', 'Nellore', 'Rajahmundry', 'Tirupati', 'Vijayawada', 'Visakhapatnam', 'Warangal'];
...
async function fetchAndDisplayCityGoldPrices(city) {
    ...
    try {
        ...
        if (todayCityData && yesterdayCityData) {
            const goldPricesDiv = document.getElementById('goldPricesAndhra');
            goldPricesDiv.innerHTML = `
                <div class="mb-8">
                    <p class="mb-4">Get real-time gold prices for Andhra 24K, 22K, and 18K Gold per gram in INR.</p>
