...
async function fetchAndDisplayCityGoldPrices(city) {
    ...
    try {
        ...
        const todayCityData = todayPrices.find(price => price.City.toLowerCase() === city.toLowerCase());
        const yesterdayCityData = yesterdayPrices.find(price => price.City.toLowerCase() === city.toLowerCase());

        if (todayCityData && yesterdayCityData) {
            goldPrices = todayPrices;
            populateGoldSidebar();
            addGoldPriceSidebar();
            displayCityGoldPrices(todayCityData, yesterdayCityData);
            const goldPricesDiv = document.getElementById('goldPrices');
            goldPricesDiv.innerHTML = `
                <div class="mb-8">
                    <p class="mb-4">Get real-time gold prices for ${city} 24K, 22K, and 18K Gold per gram in INR.</p>
...
function addGoldPriceSidebar() {
    const sidebarContainer = document.getElementById('sidebarContainer');

    if (!sidebarContainer) return;

    const sidebarHTML = `
        <div class="mt-1 bg-gray-50 rounded-xl shadow-md p-4">
            <h2 class="sidebar-section-title">Gold Price-State Wise</h2>
            <ul class="space-y-4">
                ${createSidebarLink('Andhra Pradesh', 'Gold-rate-andhra.html')}
                ${createSidebarLink('Karnataka', 'Gold-rate-karnataka.html')}
                ${createSidebarLink('Maharashtra', 'Gold-rate-maharashtra.html')}
                ${createSidebarLink('Tamil Nadu', 'Gold-rate-tamilnadu.html')}
                ${createSidebarLink('Uttar Pradesh', 'Gold-rate-uttarpradesh.html')}
                ${createSidebarLink('Punjab', 'Gold-rate-punjab.html')}
                ${createSidebarLink('Rajasthan', 'Gold-rate-jaipur.html')}
                ${createSidebarLink('Delhi', 'Gold-rate-delhi.html')}
                ${createSidebarLink('Gujarat', 'Gold-rate-gujarat.html')}
                ${createSidebarLink('Odisha', 'Gold-rate-odisha.html')}
                ${createSidebarLink('West Bengal', 'Gold-rate-kolkata.html')}
                ${createSidebarLink('Goa', 'Gold-rate-goa.html')}
                ${createSidebarLink('Haryana', 'Gold-rate-gurgaon.html')}
                ${createSidebarLink('Kerala', 'Gold-rate-kerala.html')}
                ${createSidebarLink('Telangana', 'Gold-rate-telangana.html')}
                ${createSidebarLink('Assam', 'Gold-rate-assam.html')}
                ${createSidebarLink('Chhattisgarh', 'Gold-rate-raipur.html')}
                ${createSidebarLink('Madhya Pradesh', 'Gold-rate-madhyapradesh.html')}
                ${createSidebarLink('Bihar', 'Gold-rate-patna.html')}
            </ul>
        </div>
    `;
...
function createSidebarLink(stateName, url) {
    return `
        <li>
            <a href="${url}" class="hover:text-indigo-600">${stateName}</a>
        </li>
    `;
}
